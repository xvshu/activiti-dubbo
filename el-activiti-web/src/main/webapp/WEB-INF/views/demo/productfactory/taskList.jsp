...
<h1>产品工厂-审核产品-测试</h1>
				...
				<tr id="${task.id }" tid="${task.id }">
					...
					<td>
						<c:if test="${empty task.assignee }">
							<a class="claim" href="${ctx }/demo/activitiPF/task/claim/${task.id}">签收</a>
						</c:if>
						<c:if test="${not empty task.assignee }">
							<%-- 此处用tkey记录当前节点的名称 --%>
							<a class="handle" tkey='zongjian' tname='${task.name }' href="#">办理</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
	...
	<div id="zongjian" style="display: none">
		...
		<td>下一级处理人:&nbsp;<input id="nextUsers" name="nextUsers" type="text" value="1" style=" width:100px;">请用,号分割</td>
