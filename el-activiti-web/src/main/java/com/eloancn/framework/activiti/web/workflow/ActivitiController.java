...
public class ActivitiController {
     ...
     * @return
     */
    @RequestMapping(value = "/process-list/findall",produces="application/json;charset=UTF-8")
    @ResponseBody
    public Page<ELProcessDefinitionEntity> findall(Integer page, Integer rows , HttpServletRequest request) {

        logger.info("=process-list.finishedall=> page:{} rows:{}",page,rows);
        ...
        Page<ELProcessDefinitionEntity> pageResult = new Page<ELProcessDefinitionEntity>(PageUtil.PAGE_SIZE);
        pageResult.setPageNo(page);
        pageResult.setPageSize(rows);

        ProcessDefinitionQuery processDefinitionQuery = repositoryService.createProcessDefinitionQuery().orderByDeploymentId().desc();
        List<ProcessDefinition> processDefinitionList = processDefinitionQuery.listPage(pageResult.getFirst() - 1, pageResult.getPageSize());
        List<ELProcessDefinitionEntity> elProcessDefinitionEntities = new ArrayList<>();


        /**
         * 因为流程引擎的命令模式生命周期在查询完成结束，会爆出null指针错误，所以做转换
         * bug部分代码：
         * if (!isIdentityLinksInitialized) {
         *  definitionIdentityLinkEntities = Context
         *  .getCommandContext() 此处返回null
         *
         */
        for(ProcessDefinition onep : processDefinitionList){
            elProcessDefinitionEntities.add(ELProcessDefinitionEntity.covert(onep));
        }

        pageResult.setTotalCount(processDefinitionQuery.count());
        pageResult.setResult(elProcessDefinitionEntities);
        logger.info("=process-list.finishedall=>{}", JSON.toJSONString(pageResult));
     ...
     */
    @RequestMapping(value = "/process/delete",produces="text/html;charset=UTF-8" )
    @ResponseBody
    public String delete(@RequestParam("deploymentId") String deploymentId) {
        repositoryService.deleteDeployment(deploymentId, true);
        return "部署id"+deploymentId+"的流程删除成功！";
    }
...
//    Context.setProcessEngineConfiguration(defaultProcessEngine.getProcessEngineConfiguration());
     ...
     * 挂起、激活流程实例
     */
    @RequestMapping(value = "processdefinition/update/{state}/{processDefinitionId}",produces="text/html;charset=UTF-8")
    @ResponseBody
    public String updateState(@PathVariable("state") String state, @PathVariable("processDefinitionId") String processDefinitionId,
        ...
        logger.info("=updateState=> state:{} processDefinitionId:{}",state,processDefinitionId);

        String resultStr="";
        if (state.equals("active")) {
            logger.info("=updateState=> active processDefinitionId:"+processDefinitionId);
            repositoryService.activateProcessDefinitionById(processDefinitionId, true, null);
            resultStr="已激活ID为[" + processDefinitionId + "]的流程定义。";

        } else if (state.equals("suspend")) {
            logger.info("=updateState=> suspend processDefinitionId:"+processDefinitionId);
            repositoryService.suspendProcessDefinitionById(processDefinitionId, true, null);
            resultStr="已挂起ID为[" + processDefinitionId + "]的流程定义。";
        }else{
            logger.info("=updateState=>do nothing status is {} processDefinitionId:{}",state,processDefinitionId);
        }
        return resultStr;
    }
     ...
     * 查询流程实例历史记录
     */
    @RequestMapping(value = "processinstance/query/history/{processInstanceId}")
    public String queryHistory( @PathVariable("processInstanceId") String processInstanceId,
                              RedirectAttributes redirectAttributes) {
        redirectAttributes.addAttribute("processInstanceId",processInstanceId);
        return "/workflow/finished-history";
    }
    ...
    public Page<ELHistoricTaskInstance> queryHistoryFindAll( @PathVariable("processInstanceId") String processInstanceId,Integer page, Integer rows ,
        ...
        HistoricTaskInstanceQuery hisQuery =historyService
